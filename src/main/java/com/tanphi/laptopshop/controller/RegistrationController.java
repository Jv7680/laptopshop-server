...
public class RegistrationController {
    ...
    @PostMapping("/ggoauth")
    public ResponseEntity<?> registerOauth2User(@Valid @RequestBody RegistrationRequestGoogleOauth request, BindingResult bindingResult)
    {
        if(bindingResult.hasErrors())
        {
            return new ResponseEntity<>(bindingResult.getAllErrors(),HttpStatus.NOT_ACCEPTABLE);
        }
        authenticationMapper.registerOauth2User(request);
        ApiResponse apiResponse=new ApiResponse();
    	apiResponse.setMessage("Tạo tài khoản bằng Google thành công.");
        return ResponseEntity.ok(apiResponse);
    }

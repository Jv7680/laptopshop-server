...
import com.tanphi.laptopshop.request.authen.AuthenticationRequestGgOauth;
...
public class AuthenticationController {
    ...
    @PostMapping("/loginggoauth")
    public ResponseEntity<?> loginggoauth(@Valid @RequestBody AuthenticationRequestGgOauth request, BindingResult bindingResult) {
        if(bindingResult.hasErrors())
        {
            return new ResponseEntity<>(bindingResult.getAllErrors(), HttpStatus.NOT_ACCEPTABLE);
        }
        try {
        	customerauthenticationproviderservice.authenticateggoauth(new UsernamePasswordAuthenticationToken(request.getUsername(), null));
            return ResponseEntity.ok(authenticationMapper.loginggoauth(request.getUsername()));
        } catch (AuthenticationException e) {
        	  ApiResponse apiResponse=new ApiResponse();
              apiResponse.setMessage("Nhập sai tên tài khoản");
              return ResponseEntity.ok(apiResponse);
        }
    }
//    @PostMapping("/loginggoauth")
...
//            return ResponseEntity.ok(authenticationMapper.loginggoauth(request.getUsername()));
//        } catch (AuthenticationException e) {
//            return ResponseEntity.ok("Không tìm thấy tài khoản Google");
//        }
//    }
    @PostMapping("/forgot")

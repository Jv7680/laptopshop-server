...
public class AuthenticationMapper {
    ...
    public AuthenticationResponse loginggoauth(String username) {
        Map<String, String> resultMap = authenticationService.loginggoauth(username);
        AuthenticationResponse response = new AuthenticationResponse();
        response.setId(Integer.parseInt(resultMap.get("id")));
        response.setUsername(resultMap.get("username"));
        response.setToken(resultMap.get("token"));
    ...
    public void registerOauth2User(RegistrationRequestGoogleOauth request) {
        authenticationService.registerOauth2User(request);
    }

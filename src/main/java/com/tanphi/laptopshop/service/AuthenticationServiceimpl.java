...
//        String imageLink=account;
	...
	@Override
	public Map<String, String> loginggoauth(String username) {
	    Accounts account = accountsRepo.findAccountsByUsername(username);
	    String id = account.getAccountId().toString();
	    String userRole;
	    if (account.getRoles() == Roles.ADMIN.getCode()) {
	    ...
	    if (account.getProvider() == AuthProvider.GOOGLE.getCode()) {
	        token = jwtProvider.createToken(username, userRole);
	    }
	    ...
	    if (token != null) {
	        ...
	        response.put("message", "Tài khoản của bạn không phải là tài khoản Google");
	        return response;
	    }
	}
	...
	public void registerUser(RegistrationRequest request) {
		...
		accountRegister.setIsdeleted(IsDeleteStatus.NO.getCode());
	...
	@Override
	public Accounts registerOauth2User(RegistrationRequestGoogleOauth request) {
		List<Accounts> list = accountsRepo.findAll();
		for (Accounts accountsEntity : list) {
			if (accountsEntity.getUsername().equals(request.getGmail())
					|| accountsEntity.getGmail().equals(request.getGmail())) {
		...
		Accounts accountRegisterGgoauth = new Accounts();
		try
		{
			accountRegisterGgoauth.setUsername(request.getGmail());
			accountRegisterGgoauth.setGmail(request.getGmail());
			accountRegisterGgoauth.setFirstname(request.getFirstname());
			accountRegisterGgoauth.setLastname(request.getLastname());
			accountRegisterGgoauth.setLastname(request.getLastname());
			accountRegisterGgoauth.setAddress(request.getAddress());
			accountRegisterGgoauth.setPhonenumber(request.getPhonenumber());
			accountRegisterGgoauth.setActiveAccount(ActiveAccountStatus.ACTIVE.getCode());
			accountRegisterGgoauth.setIsdeleted(IsDeleteStatus.NO.getCode());
			accountRegisterGgoauth.setRoles(Roles.CUSTOMER.getCode());
			accountRegisterGgoauth.setProvider(AuthProvider.GOOGLE.getCode());
			accountsRepo.save(accountRegisterGgoauth);
		}
		catch(Exception ex)
		{
			ex.printStackTrace();
